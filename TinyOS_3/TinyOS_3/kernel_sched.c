...
rlnode SCHED[MAX_PRIORITIES]; /* The scheduler queue */
...
static TCB* sched_queue_select(TCB* current)
{
	...
	if(i!=-1){//if i==-1 there are no non-empty lists
		rlnode* sel = rlist_pop_front(&SCHED[i]);//Get the headQUEUE of the SCHED list 
		next_thread = sel->tcb;
	}else {
		next_thread = NULL;
	}//When the list is empty, next_thread is NULL 
...
void yield(enum SCHED_CAUSE cause)
{
	...
	//ftiaxnei to priority twn Threads 
	switch(cause){

			//prwti periptwsi to thread na exei kanei to Quantum tou ara miwnetai to priority tou
			case(SCHED_QUANTUM):
			...
			//deuteri periptwsi to thread na min exei kanei to Quantum tou ara au3anei to priority tou
			case(SCHED_IO):
				if(current->Priority < (MAX_PRIORITIES-1) ){
					current->Priority++;
					break;
				}
			...
			case(SCHED_MUTEX):
				if(SCHED_MUTEX == current->last_cause)
					if(current->Priority>0){
						current->Priority--;
						break;
					}
	...
	//se periptwsi pou to num of Yields ftasei sto max prepei na ginei boost 
	if(numofYields==MAX_YIELDS){

	numofYields = 0;//Resetting the yield counter
	rlnode* node;
	for (int i = MAX_PRIORITIES-2; i>=0 ; i--){
